module defai_agent::axis_vault {
    ...
    public struct StrategyBudgetReconciled has copy, drop {
        vault_id: ID,
        ticket_id: ID,
        sequence: u64,
        original_allocated_amount: u64,
        final_allocated_amount: u64,
        refunded_amount: u64,
        available_liquidity_after: u64,
        timestamp_ms: u64,
    }
    ...
    /// Returns unused range-mint budget back to the vault while keeping the
    /// strategy ticket alive with the final deployed premium amount.
    public fun reconcile_range_position_budget<DUSDC>(
        _agent: &AgentCap,
        vault: &mut Vault<DUSDC>,
        mut ticket: StrategyTicket,
        unused_capital: Coin<DUSDC>,
        ctx: &TxContext,
    ): StrategyTicket {
        let refunded_amount = coin::value(&unused_capital);
        let returned_balance = coin::into_balance(unused_capital);
        let vault_id = object::id(vault);

        assert!(ticket.vault_id == vault_id, EInvalidStrategyTicket);
        assert!(ticket.allocated_amount >= refunded_amount, EAccountingUnderflow);
        assert!(vault.deployed_balance >= refunded_amount, EAccountingUnderflow);

        let original_allocated_amount = ticket.allocated_amount;
        vault.deployed_balance = vault.deployed_balance - refunded_amount;
        balance::join(&mut vault.cash_balance, returned_balance);
        ticket.allocated_amount = ticket.allocated_amount - refunded_amount;

        event::emit(StrategyBudgetReconciled {
            vault_id,
            ticket_id: object::id(&ticket),
            sequence: ticket.sequence,
            original_allocated_amount,
            final_allocated_amount: ticket.allocated_amount,
            refunded_amount,
            available_liquidity_after: balance::value(&vault.cash_balance),
